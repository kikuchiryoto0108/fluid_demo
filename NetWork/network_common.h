...
// 構造体のパディングを無効化し、送受信時のバイト数を正確に一致させる
#pragma pack(push, 1)
...
struct ObjectState {
    ...
    uint8_t  isAlive;                   // 生存フラグ（0=死亡, 1=生存）
    uint8_t  padding[3];                // アライメント用パディング
};

//============================================================
// 状態パケットのヘッダー（この後にObjectStateがobjectCount個続く）
//============================================================
...
struct ChannelInfo {
    ...
    uint32_t userCount;      // このチャンネルの接続ユーザー数
...
struct PacketBullet {
    ...
    uint32_t seq;           // シーケンス番号
    uint32_t ownerPlayerId; // 発射したプレイヤーのID
...
// UDPパケットの最大サイズ（MTU値から1400バイトに制限）
